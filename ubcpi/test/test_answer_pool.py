...
from answer_pool import offer_answer, validate_seeded_answers_simple
...
class TestAnswerPool(unittest.TestCase):
    def test_simple_algo_insert_one(self):
        ...
        offer_answer(pool, expected_answer, expected_rationale, expected_student_id, 'simple')
    ...
    def test_simple_algo_insert_max(self):
        ...
        for i in range(50):
            for cur_expected_answer in options:
                ...
                offer_answer(pool, cur_expected_answer, cur_expected_rationale,
                             cur_expected_student_id, 'simple')
        self.assertEqual(expected_pool, pool)
        ...
        with patch('random.choice', return_value="student id0optionA"):
            offer_answer(pool, options[0], "some rationale", "test student 51", 'simple')
        for i in range(50):
            for cur_expected_answer in options:
                cur_expected_student_id = "student id" + str(i) + cur_expected_answer
    ...
    def test_validate_seeded_answers_simple(self):
        options = ['optionA', 'optionB', 'optionC']
        answers = [
            {'answer': 'optionA', 'desc': 'rationale A'},
            {'answer': 'optionB', 'desc': 'rationale B'},
            {'answer': 'optionC', 'desc': 'rationale C'},
        ]

        self.assertIsNone(validate_seeded_answers_simple(answers, options))

        options = ['option1', 'optionB', 'optionC', 'optionD']

        self.assertEqual(validate_seeded_answers_simple(answers, options), "Missing option(s) ['option1', 'optionD']")
